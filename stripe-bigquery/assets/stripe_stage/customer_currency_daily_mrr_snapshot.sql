...
type: bq.sql
description: >
  Daily MRR snapshot at Stripe customer and native-currency grain. Each run
  observes the pipeline end date and appends that day to an accumulating
  history, replacing only the rows for the snapshot date being run, so
  re-running a date is idempotent. It starts from subscriptions and left
