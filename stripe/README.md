# stripe — Home Box subscription billing (recurring revenue)

`stripe` is Harborside's subscription-billing pipeline: the recurring-revenue
counterpart to the one-time `shop` pipeline. It loads the customer, product,
price, subscription, subscription-item, and invoice resources into `stripe_raw`;
builds reusable billing models in `stripe_stage`; and publishes recurring-revenue
and invoice-billing reports in `stripe_report`.

**Consumers:** Finance (invoice billings) and Growth (MRR, churn), plus the DAC
billing dashboard. It also serves as an upstream **data contract** for the `shop`
pipeline (see below).

The pipeline contains 19 assets across three layers, and is the only pipeline
here sourced through ingestr rather than a Python fixture — it demonstrates
incremental Stripe loading, `merge` and `delete+insert` snapshot strategies, and
a DAC dashboard.
...
Billing is a finance domain, so every asset carries `domains: [finance]`, cost
center `FIN-204`, and layer/tier metadata (`bronze` raw, `silver` stage, `gold`
report). Ownership splits by metric family:

- **Dana** (Finance, `dana@harborside.example`) owns the invoice billings
  report and the raw and stage models, including the stage invoice models.
- **Priya** (Growth, `priya@harborside.example`) owns the MRR, movement, and
  subscription KPI reports: `monthly_mrr_by_customer`, `monthly_mrr_movements`,
  and `monthly_subscription_kpis`.

Report tables declare a freshness SLA of `06:00 UTC`, and `pipeline.yml` routes
failures to the `#data-alerts` Slack channel.

## Cross-pipeline contract

This pipeline is an upstream data contract for the `shop` pipeline. Two stage
models publish a stable BigQuery address through a top-level `uri:`:

- `stripe_stage.subscriptions` → `bigquery://harborside-analytics.stripe_stage.subscriptions`
- `stripe_stage.customers` → `bigquery://harborside-analytics.stripe_stage.customers`

The `shop` pipeline's `shop_report.subscriber_revenue_daily` depends on the
`stripe_stage.subscriptions` URI as a blocking data contract. Because this
pipeline runs with `catchup: false`, a missed daily interval is not
automatically backfilled, so the downstream `shop` asset waits on the contract
until stripe produces that interval. That is the cross-pipeline missed-interval
self-heal scenario: the fix is to run the missing stripe interval, which then
releases the blocked `shop` report rather than letting it read stale or partial
subscription data.
...
```bash
bruin run --start-date 2023-01-01 --end-date $(date -u +%F) ./stripe
```
...
The `stripe_report` dataset provides four ready-to-query tables:
...
the BigQuery table and column metadata, and `bruin docs ./stripe`
...
Query the four tables in `stripe_report` directly, then adapt the columns to
...
All monetary values in the `stripe_stage` and `stripe_report` tables remain in
...
This pipeline already exists in the repo as `stripe/`. Configure the connections
above, set `start_date` in `pipeline.yml` to the earliest Stripe history you need,
then run the fast validation checks:

```bash
bruin validate --fast ./stripe
```
...
bruin run --full-refresh --start-date 2023-01-01 --end-date $(date -u +%F) \
  --no-validation ./stripe
```

After the initial load, run `bruin validate ./stripe` and schedule the
pipeline daily without `--full-refresh`:

```bash
bruin run --start-date $(date -u +%F) --end-date $(date -u +%F) ./stripe
```
