...
type: bq.sql
description: >
  Daily snapshot of current subscription-item MRR state. Each run observes the
  pipeline end date and appends that day to an accumulating history, replacing
  only the rows for the snapshot date being run, so re-running a date is
  idempotent. It cannot reconstruct historic state from mutable Stripe
  subscription records, so history only builds up from the first run onward.
