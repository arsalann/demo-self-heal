"""@bruin
name: webevents_raw.pageviews
type: python
image: python:3.11
connection: gcp-default
description: |
  Raw website pageview events. Source-faithful and append-only. One row per
  pageview: session, user, geo, browser, device, path, and timestamps.

  This asset emulates the analytics event source with a deterministic generator
  (~40k events/day across 6 countries, 4-5 browsers, 3 devices, 8 paths) so the
  demo runs offline. It seeds designed incidents so the repo doubles as a
  self-heal exercise:

    1. Traffic spike (single-dimension driver) — on 2026-05-18 country=ID gets 8x
       its normal share; daily pageviews jump ~3x.
    2. New segment (enum value added) — the Arc browser appears from 2026-05-16
       at ~3% share. `known_browsers_only` on the report warns; treat as a
       contract change, not a bad row.
    3. Multi-day source gap (freshness) — for 2026-05-23..2026-05-25 the generator
       emits zero rows, as if the event source stopped.

  Source timing: the analytics vendor delivers events 4-6 hours late, so this
  asset carries an interval modifier that shifts the run's start back 6 hours.
  Without it every run would report a gap that is not really a gap.
...
meta:
  ...
  team: growth
  cost_center: MKT-330
  source: analytics event stream (emulated, 4-6h late)
  contains_incidents: "traffic-spike;new-browser-segment;source-gap"
...
columns:
  ...
  - name: inserted_at
    type: TIMESTAMP
    description: UTC time this run inserted the row. Used to deduplicate replays downstream.
    nullable: false
...
def materialize():
    ...
    while current <= end:
        if SOURCE_GAP_START_DATE <= current <= SOURCE_GAP_END_DATE:
            print(f"[webevents] {current}: simulated source stall — returning 0 rows")
        else:
    ...
    print(f"[webevents] generated {len(df):,} events across {df['event_date'].nunique()} days")
    return df
